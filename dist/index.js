...
(() => {
  ...
  var swipers = () => {
    ...
    const swipersEl = document.querySelectorAll("[swiper-option]");
    if (!swipersEl)
      return;
    swipersEl.forEach((el) => {
      const attrEl = el.getAttribute("swiper-option");
      if (attrEl === "home-articles") {
        const elPrev = document.querySelector(`[swiper-prev="${attrEl}"]`);
        const elNext = document.querySelector(`[swiper-next="${attrEl}"]`);
